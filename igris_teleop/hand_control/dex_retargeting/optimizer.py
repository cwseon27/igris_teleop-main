...
class VectorOptimizer(Optimizer):
    ...
    def get_objective_function(self, target_vector: np.ndarray, fixed_qpos: np.ndarray, last_qpos: np.ndarray):
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            result = huber_distance.cpu().detach().item()
            # SLSQP's objective and gradient must include the same temporal
            # regularizer; its derivative is added to grad_qpos below.
            result += self.norm_delta * float(np.sum((x - last_qpos) ** 2))
