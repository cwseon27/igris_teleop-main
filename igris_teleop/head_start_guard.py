...
from .core.math.transforms import (
...
def build_candidate_head_target(
    ...
    relative = common_frame_relative_pose(now, home)
    return apply_common_frame_relative_pose(relative, robot_home)
...
def evaluate_head_start_guard(
    ...
    *,
    ...
    relative = common_frame_relative_pose(
        np.asarray(vr_head_now, dtype=np.float64).reshape(4, 4),
        np.asarray(vr_head_home, dtype=np.float64).reshape(4, 4),
    )

    pos_err_m = float(np.linalg.norm(relative[:3, 3]))
    rot_err_deg = float(rotation_error_deg(relative[:3, :3], np.eye(3)))
    yaw_correction = float(yaw_correction_deg(relative[:3, :3], np.eye(3)))
