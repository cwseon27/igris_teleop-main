...
from .waist_task_transition import WaistTransitionParams
...
class IKConfig:
    ...
    waist_transition: WaistTransitionParams = WaistTransitionParams()
...
def _apply_env_overrides(cfg: IKConfig, profile: str | None = None) -> IKConfig:
    ...
    current_transition = cfg.waist_transition
    transition_updates: dict[str, Any] = {}
    transition_prefix = f"IGRIS_IK_{profile_part}WAIST_TRANSITION_"
    for field in fields(current_transition):
        env_name = f"{transition_prefix}{field.name.upper()}"
        if env_name in os.environ:
            transition_updates[field.name] = _coerce_like(
                os.environ[env_name],
                getattr(current_transition, field.name),
            )
...
class IGRIS_C_UpperIKProxSuite:
    ...
    def _add_end_effector_frames(self) -> None:
        ...
        # body_link already uses robot-forward +X. Keep these virtual axes
        # aligned with it instead of introducing a fixed -90 degree yaw.
        body_target_rot = np.eye(3, dtype=np.float64)

        def add_body_target_frame(frame_name: str, translation: np.ndarray) -> None:
            if model.existFrame(frame_name):
                return
            offset = pin.SE3(body_target_rot, translation)
            if model.existFrame("body_link"):
                ...
                if t_parent == 0:
                    raise ValueError(
                        f"Cannot resolve {frame_name}: body_link and 2_Joint_Waist_Yaw not found"
                    )
                model.addFrame(pin.Frame(frame_name, t_parent, offset, pin.FrameType.OP_FRAME))

        self.torso_frame_name = "T_ee"
        add_body_target_frame(self.torso_frame_name, np.array([0.05, 0.0, 0.3]))

        # A distinct chest-mounted controller target, approximately 20 cm in
        # front of body_link and 20 cm below the virtual head target.
        self.chest_frame_name = "C_ee"
        add_body_target_frame(self.chest_frame_name, np.array([0.20, 0.0, 0.3]))
        ...
        self.Chest_id = model.getFrameId(self.chest_frame_name, pin.FrameType.OP_FRAME)
    ...
    def _add_frame_task(
        ...
        # Linearize the SE(3) logarithm itself. Using the raw frame Jacobian is
        # only accurate close to zero pose error and can point the QP in the
        # wrong direction after a large HMD rotation.
    ...
    def _build_qp_matrices(
        ...
        use_chest_target: bool,
        use_chest_translation_target: bool,
        chest_translation_weight: float,
        chest_rotation_weight: float,
        ...
        if use_torso_target and torso_pose is not None:
            self._add_frame_task(
                ...
                self.Torso_id,
                torso_pose,
                w.w_trans * max(0.0, float(torso_translation_weight))
                if use_torso_translation_target
                else 0.0,
                w.w_rot * max(0.0, float(torso_rotation_weight)),
            )

        if use_chest_target and chest_pose is not None:
            self._add_frame_task(
                H,
                g,
                q_current,
                self.Chest_id,
                chest_pose,
                w.w_trans * max(0.0, float(chest_translation_weight))
                if use_chest_translation_target
                else 0.0,
                w.w_rot * max(0.0, float(chest_rotation_weight)),
            )
    ...
    def commit_published_command(
        ...
        self._last_dq = np.asarray(
            clipped_command - clipped_previous,
            dtype=np.float64,
        )
    ...
    def _motion_step_bounds(
        self,
        q_reference: np.ndarray,
        q_current: np.ndarray,
        *,
        waist_motion_reference_q: np.ndarray | None = None,
        waist_delta_limit: float | np.ndarray | None = None,
        ...
        position_rate_lower = total_lower.copy()
        position_rate_upper = total_upper.copy()
        bound_info: dict[str, Any] = {
            "waist_bound_active": False,
            "waist_acceleration_override": False,
            "waist_delta_limit": None,
        }
        ...
        if (waist_motion_reference_q is None) != (waist_delta_limit is None):
            raise ValueError(
                "waist_motion_reference_q and waist_delta_limit must be provided together"
            )
        if waist_motion_reference_q is not None:
            waist_reference = np.asarray(
                waist_motion_reference_q,
                dtype=np.float64,
            ).reshape(-1)
            if waist_reference.shape == (model.nq,):
                waist_reference = waist_reference[self._waist_slice]
            elif waist_reference.shape != (self._waist_dof,):
                raise ValueError(
                    "waist motion reference must be a full q vector or a "
                    f"{self._waist_dof}-element waist vector"
                )
            if not np.all(np.isfinite(waist_reference)):
                raise ValueError("waist motion reference must be finite")

            limit_arr = np.asarray(waist_delta_limit, dtype=np.float64)
            if limit_arr.ndim == 0:
                waist_limit = np.full(
                    self._waist_dof,
                    float(limit_arr),
                ...
                waist_limit = limit_arr.reshape(-1)
                if waist_limit.shape != (self._waist_dof,):
                    raise ValueError(
                        f"waist delta limit shape {waist_limit.shape} "
                        f"!= ({self._waist_dof},)"
                    )
            if not np.all(np.isfinite(waist_limit)) or np.any(waist_limit < 0.0):
                raise ValueError("waist delta limit must be finite and nonnegative")

            waist_lower_q = lower_q[self._waist_slice]
            waist_upper_q = upper_q[self._waist_slice]
            if np.any(waist_reference < waist_lower_q - 1e-10) or np.any(
                waist_reference > waist_upper_q + 1e-10
            ):
                raise ValueError("waist motion reference is outside joint limits")

            waist_abs_lower = np.maximum(
                waist_reference - waist_limit,
                waist_lower_q,
            )
            waist_abs_upper = np.minimum(
                waist_reference + waist_limit,
                waist_upper_q,
            )
            gate_lower = waist_abs_lower - q_reference[self._waist_slice]
            gate_upper = waist_abs_upper - q_reference[self._waist_slice]

            position_gate_lower = np.maximum(
                position_rate_lower[self._waist_slice],
                gate_lower,
            )
            position_gate_upper = np.minimum(
                position_rate_upper[self._waist_slice],
                gate_upper,
            )
            if np.any(position_gate_lower > position_gate_upper + 1e-12):
                raise ValueError(
                    "waist command window conflicts with joint/dq limits: "
                    f"lower={position_gate_lower}, upper={position_gate_upper}"
                )

            gated_lower = np.maximum(
                total_lower[self._waist_slice],
                gate_lower,
            )
            gated_upper = np.minimum(
                total_upper[self._waist_slice],
                gate_upper,
            )
            gated_invalid = gated_lower > gated_upper + 1e-12
            if np.any(gated_invalid):
                if np.all(waist_limit <= 1e-12):
                    # An invalid/zero-confidence hold is authoritative. The
                    # command position does not jump; only the command-velocity
                    # history is stopped immediately for these waist joints.
                    gated_lower = position_gate_lower
                    gated_upper = position_gate_upper
                    bound_info["waist_acceleration_override"] = True
                else:
                    raise ValueError(
                        "waist command window conflicts with acceleration/jerk "
                        f"limits: lower={gated_lower}, upper={gated_upper}"
                    )

            total_lower[self._waist_slice] = gated_lower
            total_upper[self._waist_slice] = gated_upper
            bound_info["waist_bound_active"] = True
            bound_info["waist_delta_limit"] = waist_limit.copy()
        ...
        return step_lower, step_upper, bound_info
    ...
    def solve_ik(
        ...
        chest_pose=None,
        use_chest_target: bool = False,
        use_chest_translation_target: bool = True,
        chest_target_weight: float = 1.0,
        chest_translation_weight: float | None = None,
        chest_rotation_weight: float | None = None,
        waist_motion_reference_q=None,
        waist_delta_limit: float | np.ndarray | None = None,
        ...
        if self.init_data.shape == q_init.shape:
            ...
            if (
                bool(commit_state)
                and seed_jump > max(0.03, 0.5 * float(self.cfg.rate_limit.dq_max))
            ):
        ...
        current_chest_pose = self._current_pose_matrix(q_init, self.Chest_id)
        ...
        if chest_pose is None:
            chest_target_pose = current_chest_pose
        else:
            chest_target_pose = self._sanitize_pose_matrix(chest_pose, current_chest_pose)
        ...
        chest_trans_w = (
            max(0.0, float(chest_target_weight))
            if chest_translation_weight is None
            else max(0.0, float(chest_translation_weight))
        )
        chest_rot_w = (
            max(0.0, float(chest_target_weight))
            if chest_rotation_weight is None
            else max(0.0, float(chest_rotation_weight))
        )
        ...
        targets_for_telemetry = [
            ...
            ("chest", self.Chest_id, chest_target_pose, bool(use_chest_target)),
        ]
        ...
        try:
            ...
            for _ in range(sqp_iters):
                step_lower, step_upper, step_bound_info = self._motion_step_bounds(
                    q_init,
                    q_work,
                    waist_motion_reference_q=waist_motion_reference_q,
                    waist_delta_limit=waist_delta_limit,
                )
                waist_bound_info["waist_bound_active"] = bool(
                    waist_bound_info["waist_bound_active"]
                    or step_bound_info["waist_bound_active"]
                )
                waist_bound_info["waist_acceleration_override"] = bool(
                    waist_bound_info["waist_acceleration_override"]
                    or step_bound_info["waist_acceleration_override"]
                )
                if step_bound_info["waist_delta_limit"] is not None:
                    waist_bound_info["waist_delta_limit"] = np.asarray(
                        step_bound_info["waist_delta_limit"],
                        dtype=np.float64,
                    ).copy()
                H, g, C, l, u, build_info = self._build_qp_matrices(
                    ...
                    use_chest_target=bool(use_chest_target),
                    use_chest_translation_target=bool(use_chest_translation_target),
            ...
            final_lower, final_upper, final_bound_info = self._motion_step_bounds(
                q_init,
                q_init,
                waist_motion_reference_q=waist_motion_reference_q,
                waist_delta_limit=waist_delta_limit,
            ...
            waist_bound_info["waist_bound_active"] = bool(
                waist_bound_info["waist_bound_active"]
                or final_bound_info["waist_bound_active"]
            )
            waist_bound_info["waist_acceleration_override"] = bool(
                waist_bound_info["waist_acceleration_override"]
                or final_bound_info["waist_acceleration_override"]
            )
            if final_bound_info["waist_delta_limit"] is not None:
                waist_bound_info["waist_delta_limit"] = np.asarray(
                    final_bound_info["waist_delta_limit"],
                    dtype=np.float64,
                ).copy()
            ...
            if bool(commit_state):
                self.commit_published_command(sol_q, q_init)
            ...
            target_residual_info = {
                f"{name}_{phase}": float(value)
                for phase, residuals in (
                    ("before", residuals_before),
                    ("after", residuals_after),
                )
            ...
            self._last_solve_info = {
                ...
                "waist_bound_active": bool(
                    waist_bound_info["waist_bound_active"]
                ),
                "waist_acceleration_override": bool(
                    waist_bound_info["waist_acceleration_override"]
                ),
                "waist_delta_limit": (
                    np.asarray(
                        waist_bound_info["waist_delta_limit"],
                        dtype=np.float64,
                    ).tolist()
                    if waist_bound_info["waist_delta_limit"] is not None
                    else None
                ),
    ...
    def get_ee_poses(self, q):
        ...
        pin.updateFramePlacements(self.reduced_robot.model, self.reduced_robot.data)
        return (
            self.reduced_robot.data.oMf[self.L_hand_id].copy(),
            self.reduced_robot.data.oMf[self.R_hand_id].copy(),
            self.reduced_robot.data.oMf[self.Head_id].copy(),
        )
    ...
    def get_torso_pose(self, q):
        ...
        pin.forwardKinematics(self.reduced_robot.model, self.reduced_robot.data, q_arr)
        pin.updateFramePlacements(self.reduced_robot.model, self.reduced_robot.data)
        return self.reduced_robot.data.oMf[self.Torso_id].copy()

    def get_chest_pose(self, q):
        q_arr = np.asarray(q, dtype=np.float64).reshape(-1)
        pin.forwardKinematics(self.reduced_robot.model, self.reduced_robot.data, q_arr)
        pin.updateFramePlacements(self.reduced_robot.model, self.reduced_robot.data)
        return self.reduced_robot.data.oMf[self.Chest_id].copy()
